...
import com.bearya.intelliscreen.data.bean.PageJ
import com.bearya.intelliscreen.databinding.ModelPAAALMRBinding
...
class JFragment : Fragment() {
    ...
    companion object {
        fun newInstance(item: PageJ?): JFragment
            = JFragment().apply { arguments = bundleOf("item" to item) }
    }

    private lateinit var bindView: ModelPAAALMRBinding

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        bindView = ModelPAAALMRBinding.inflate(inflater, container, false)
        return bindView.root
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        val item = arguments?.getSerializable("item") as? PageJ?
        ...
        bindView.audioMiddle.onFocusChangeListener = View.OnFocusChangeListener { _, hasFocus ->
            bindView.audioMiddle.setBorderWidth(if (hasFocus) 3 else 0)
            bindView.playMiddle.visibility = if (hasFocus) View.VISIBLE else View.INVISIBLE
            if (hasFocus) Music.stop() else bindView.playMiddle.setImageResource(R.drawable.music_play)
        }
        ...
        bindView.audioMiddle.setOnClickListener {
            Music.autoAudio(Storage.getUsbDir(requireContext()) + item?.audioB) { isPlaying , message ->
                bindView.playMiddle.setImageResource(if (isPlaying) R.drawable.music_stop else R.drawable.music_play)
            }
        }
